...
app.all(
  "/api/auth/*",
  proxyMiddleware(config.AUTH_SERVICE_URL, "/api/auth", logger)
);

// Proxy to campaign-service
app.all(
  "/api/campaigns/*",
  proxyMiddleware(config.CAMPAIGN_SERVICE_URL, "/api/campaigns", logger)
);

// Proxy to donation-service
app.all(
  "/api/donations/*",
  proxyMiddleware(config.DONATION_SERVICE_URL, "/api/donations", logger)
);

// Proxy to payment-service
app.all(
  "/api/payments/*",
  proxyMiddleware(config.PAYMENT_SERVICE_URL, "/api/payments", logger)
);

// Proxy to totals-service
app.all(
  "/api/totals/*",
  proxyMiddleware(config.TOTALS_SERVICE_URL, "/api/totals", logger)
);

// Proxy to chat-service
app.all(
  "/api/chat/*",
  proxyMiddleware(config.CHAT_SERVICE_URL, "/api/chat", logger)
);
